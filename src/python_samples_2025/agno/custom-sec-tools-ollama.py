...
class OllamaAgent(Agent):
    ...
    def run(self, message: str, **kwargs) -> str:  # Respeta la firma de la clase base
        """Envía un prompt a Ollama y devuelve la respuesta."""
        payload = {
            "model": self.model,
            "prompt": message,  # Usamos el parámetro 'message' del framework
            "stream": False
        }
        try:
...
def is_ollama_running():
    """Verifica si Ollama está en ejecución haciendo una petición HTTP."""
    try:
...
def start_ollama():
    ...
    print("Ollama no está corriendo. Iniciándolo...")
    subprocess.Popen(["ollama", "serve"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    for _ in range(10):  # Espera hasta 10 segundos verificando cada segundo
        time.sleep(1)
        if is_ollama_running():
            print("Ollama iniciado correctamente.")
            return
    print("Advertencia: Ollama podría no haberse iniciado correctamente.")
...
def list_ollama_models(retries=5, delay=3):
    """Obtiene la lista de modelos disponibles en Ollama con reintentos."""
    for attempt in range(retries):
        try:
            result = subprocess.run(["ollama", "list"], capture_output=True, text=True, check=True)
            models_data = result.stdout.split("\n")[1:]
            models = [line.split()[0] for line in models_data if line]
            if models:
                return models
...
if __name__ == "__main__":
    ...
    '''
    Right now, it only works with llama3.2, so i will hardcode the id
    '''
    agent = Agent(
        model=Ollama(id=selected_model, client=OllamaClient()),
        description=dedent("""\
